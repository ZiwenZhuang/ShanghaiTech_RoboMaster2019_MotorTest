...
void _init_() {
  ...
  for(int i=0; i<4; i++)
  {
	...
	PWM_SetDuty(&htim5, TIM_CHANNEL_1, pwm_duty);
...
void _loop_() {
      ...
      // remote_control.ch4*8000; // 摇杆度数
			...
			// set motor target
			mecanum_calc(remote_control.ch3 * -3,
									remote_control.ch4 * 3,
									remote_control.ch1 / 10,
									motor_speed_target);
			...
			// set pitch target
			pitch_pwm_duty = rc2pitchPWM(remote_control.ch1);
    }
